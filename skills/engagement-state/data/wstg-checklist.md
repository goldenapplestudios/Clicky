<!--
Vendored from the OWASP Web Security Testing Guide checklist:
...
Retrieved 2026-08-26. Licensed CC BY-SA 4.0 by the OWASP Foundation.

This is the AUTHORITATIVE source for the test IDs and names used by
skills/engagement-state/scripts/coverage-ledger.sh. Do not hand-edit test
names here or in the generated catalog - re-fetch this file and re-run
tools/generate-coverage-catalog.py instead. Hand-written test names drift
from the official ones (an earlier revision of the ledger paraphrased 25 of
36 entries from memory and mislabelled WSTG-APIT-01 "Test GraphQL" when the
official name is "API Reconnaissance"), and a client-facing report that
cites a WSTG ID against the wrong test name is worse than one that cites no
ID at all.
-->
