...
- Multi-CLI portability underway: OpenCode support is built and live-verified (real generated agents, real adversarial permission test, real end-to-end scan against a live target - see `tools/generate-cli-targets.py`), generated from Claude Code's own agent definitions rather than a hand-maintained parallel copy - the same source-of-truth-plus-generation pattern `NeoTheCapt/RedteamAgent` (not otherwise profiled in this document) uses for its own Claude Code/OpenCode/Codex support. Codex CLI and Copilot CLI targets are planned, not yet built.
