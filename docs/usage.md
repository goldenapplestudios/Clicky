...
Neither `/pentest` nor any of the 11 agents it dispatches has a direct `Bash`/`Read`/`Write`/`Grep`/`WebFetch` grant anymore - every action, including the orchestrating command's own session/scope bookkeeping, goes through the Clicky MCP gateway's tools instead (see `skills/mcp-gateway/SKILL.md`). `execute_command` is what actually runs `nmap`/`sqlmap`/`hydra`/etc. underneath, resolving `TARGET_n`-style tokens to real values first. Credential-attack tooling specifically (`hydra`, `medusa`, `ncrack`, `patator`, `crowbar`, netexec/crackmapexec sprays, Metasploit `*_login` modules) is refused at this layer unless the session holds a `credential_attack` authorization recorded by `skills/engagement-state/scripts/technique-gate.sh` - see `skills/engagement-state/SKILL.md` for the three preconditions and why the gate is enforced here rather than requested in an agent prompt.
