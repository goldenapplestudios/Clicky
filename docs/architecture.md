...
Most agent files set `model: inherit` in their frontmatter (verify with `grep -n "^model:" agents/*.md`), so they run on whatever model the invoking Claude Code session (the orchestrator) is already using. The **offensive** specialists - `privesc-agent`, `exploit-agent`, `loot-agent`, `cloud-recon-agent` - are the exception: they pin `model: sonnet`. This is deliberate and load-bearing under Anthropic's real-time cyber safeguards. Those safeguards gate high-risk dual-use work (vulnerability exploitation, credential attacks) even for a CVP-approved organization, and the gating is *stricter on the strongest models* - on a live engagement, offensive subagents that inherited the orchestrator's Opus tier were terminated mid-run with a `[cyber]` error while the orchestrator's own calls were not. Pinning the offensive agents to a capable-but-less-strict, CVP-covered tier keeps the specialists alive so the multi-agent design can actually run. Analysis/planning/report agents keep `model: inherit`. An operator can override any agent's tier with `tools/clicky-setup.sh --advanced` (writes `~/.clicky/agent-models.json`, consumed by `tools/advanced_agent_assignment.py` + the per-CLI generator). There is no `temperature:` field in Claude Code's subagent frontmatter schema - it was never a real capability here.
