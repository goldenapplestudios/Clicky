...
<!--
  Generated by tools/generate-cli-targets.py from agents/source-analyzer-agent.md.
  ...
  Scoped to these skills in Claude Code (skills: frontmatter): source-code-analysis, credential-harvesting, target-validation, session-management, tool-management. OpenCode has no confirmed per-agent skill-allowlist mechanism -
